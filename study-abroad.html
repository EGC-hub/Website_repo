<!DOCTYPE html>
<html lang="en">

<!-- Mirrored from html.creativegigstf.com/charles/service-details.html by HTTrack Website Copier/3.x [XR&CO'2014], Thu, 18 Jul 2024 01:06:06 GMT -->

<head>
<!-- Google tag (gtag.js) -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-P0RM2XGQ2R"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'G-P0RM2XGQ2R');
</script>
    <meta charset="UTF-8">
    <!-- For IE -->
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <!-- For Resposive Device -->
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <!-- For Window Tab Color -->
    <!-- Chrome, Firefox OS and Opera -->
    <meta name="theme-color" content="#061948">
    <!-- Windows Phone -->
    <meta name="msapplication-navbutton-color" content="#061948">
    <!-- iOS Safari -->
    <meta name="apple-mobile-web-app-status-bar-style" content="#061948">
    <title>Study Abroad Consultancy: Expert Guidance & Top Universities</title>
	<!-- Description Tag -->
	<meta name="description"
	content="Fulfill your study abroad dreams with us. We offer expert guidance, personalized support, and access to prestigious universities all over the world.">
	<!-- Language Tag -->
	<meta property="og:locale" content="en_US">
	<!-- Open Graph Tags -->
	<meta property="og:type" content="website" />
	  <meta property="og:title" content="Study Abroad Consultancy: Expert Guidance & Top Universities">
	  <meta property="og:description"
		  content="Fulfill your study abroad dreams with us. We offer expert guidance, personalized support, and access to prestigious universities all over the world.">
	  <meta property="og:url" content="https://euroglobalconsultancy.com/study-abroad.html">
	  <meta property="og:site_name" content="Euro Global Consultancy">
	<!-- Canonical Tag -->
	<link rel="canonical" href="https://www.euroglobalconsultancy.com/study-abroad.html" />
	<!-- Robots Tag -->
	<meta name="robots" content="index, follow">
    <!-- Favicon -->
    <link rel="icon" type="image/png" sizes="56x56" href="images/logo/logo-2.1.ico">
    <!-- Main style sheet -->
    <link rel="stylesheet" type="text/css" href="css/style.css">
    <!-- responsive style sheet -->
    <link rel="stylesheet" type="text/css" href="css/responsive.css">

    <!-- Fix Internet Explorer ______________________________________-->
    <!--[if lt IE 9]>
			<script src="http://html5shiv.googlecode.com/svn/trunk/html5.js"></script>
			<script src="vendor/html5shiv.js"></script>
			<script src="vendor/respond.js"></script>
		<![endif]-->
</head>

<body>
    <div class="main-page-wrapper">

        <!-- ===================================================
				Loading Transition
			==================================================== -->
        <div id="loader-wrapper">
            <div id="loader"></div>
        </div>



        <!-- 
			=============================================
				Theme Header One
			============================================== 
			-->
        <header class="header-one">
            <div class="top-header">
                <div class="container clearfix">
                    <div class="logo float-left">
                        <a href="index.html"><img src="images/logo/logo.webp" alt="" /></a>
                    </div>
                    <div class="address-wrapper float-right">
                        <ul>
                            <li class="address first-icon">
                                <i class="icon flaticon-placeholder"></i>
                                <h6>Address:</h6>
                                <p>
                                    Royal house building, <br />
                                    Office No M08-05, <br />Mezzanine floor,<br />
                                    Al Wuheida Road,<br />
                                    Hor Al Anz East, Dubai.
                                </p>
                            </li>
                            <li class="address">
                                <i class="icon flaticon-multimedia"></i>
                                <h6>Mail us:</h6>
                                <a href="mailto:enquiry@euroglobalconsultancy.com">enquiry@euroglobalconsultancy.com</a>
                            </li>
                            <li class="quotes"><a href="contact.html">BOOK AN APPOINTMENT</a></li>
                        </ul>
                    </div>
                    <!-- /.address-wrapper -->
                </div>
                <!-- /.container -->
            </div>
            <!-- /.top-header -->

            <div class="theme-menu-wrapper">
                <div class="container">
                    <div class="bg-wrapper clearfix">
                        <!-- ============== Menu Warpper ================ -->
                        <div class="menu-wrapper float-left">
                            <nav id="mega-menu-holder" class="clearfix">
                                <ul class="clearfix">
                                    <li><a href="index.html">Home</a></li>
                                    <li><a href="about.html">About Us</a></li>
                                    <li>
                                        <a href="IT-services.html">IT Services</a>
                                    </li>
                                    <li>
                                        <a href="training.html">Training</a>
                                        <ul class="dropdown">
                                            <li><a style="padding-right: 10px;" href="ITGRC.html">IT
                                                    Governance, Risk &
                                                    Compliance</a></li>
                                        </ul>
                                    </li>
                                    <li class="active">
                                        <a href="study-abroad.html">Study Abroad</a>
                                    </li>
                                    <li>
                                        <a href="./blogs.html">Blogs</a>
                                    </li>
                                    <li><a href="contact.html">Contact Us</a></li>
                                </ul>
                            </nav>
                            <!-- /#mega-menu-holder -->
                        </div>
                        <!-- /.menu-wrapper -->

                        <div class="right-widget float-right">
                            <ul>
                                <li class="social-icon">
                                    <ul>
                                        <li>
                                            <a href="https://www.facebook.com/EuroGlobalConsultancy"><i
                                                    class="fa fa-facebook" aria-hidden="true"></i></a>
                                        </li>
                                        <li>
                                            <a href="https://www.instagram.com/euroglobalconsultancyfzc/"><i
                                                    class="fa fa-instagram" aria-hidden="true"></i></a>
                                        </li>
                                        <li>
                                            <a href="https://api.whatsapp.com/send?phone=971544888923"><i
                                                    class="fa fa-whatsapp" aria-hidden="true"></i></a>
                                        </li>
                                        <li>
                                            <a href="https://www.linkedin.com/company/euroglobal-consultancyuae"><i
                                                    class="fa fa-linkedin-square" aria-hidden="true"></i></a>
                                        </li>
                                    </ul>
                                </li>
                            </ul>
                        </div>
                        <!-- /.right-widget -->
                    </div>
                    <!-- /.bg-wrapper -->
                </div>
                <!-- /.container -->
            </div>
            <!-- /.theme-menu-wrapper -->
        </header> <!-- /.header-one -->


        <!-- 
			=============================================
				Theme Inner Banner
			============================================== 
			-->
        <div class="theme-inner-banner section-spacing">
            <div class="overlay">
                <div class="container">
                    <h2>Study Abroad Consultancy</h2>
                </div> <!-- /.container -->
            </div> <!-- /.overlay -->
        </div> <!-- /.theme-inner-banner -->


        <!-- 
			=============================================
				Service Details
			============================================== 
			-->
        <section class="service-details section-spacing">
            <div class="container">
                <div class="row">
                    <div class="col-xl-12 col-lg-12 col-12">
                        <div class="study-abroad">
                            <h5>Study Abroad Consultants in UAE, Dubai</h5>
                            <img src="images/home/Students.jpeg" alt="">
                            <p>
                                Our study abroad consulting services are dedicated to helping you achieve your dreams of
                                pursuing an overseas education. We believe that studying abroad is an incredibly
                                valuable experience that can broaden your horizons, build your skills, and enrich your
                                perspective on the world.
                            </p>
                            <br>
                            <p>
                                Our team of experienced consultants is here to guide you through every step of the
                                process, from choosing the right program to ensuring a successful and fulfilling
                                experience. Our services begin with an initial consultation, during which we get to know
                                you and your goals for studying abroad. Based on this information, we'll work with you
                                to identify the right program and destination to fit your needs and aspirations.Once
                                you've chosen a program, we'll guide you through the application process, providing
                                expert advice and support along the way and We'll help you navigate the logistical
                                details, including visa applications and other important considerations.
                            </p>
                            <br>
                            <p>
                                At our study abroad consultants in UAE/Dubai, we're passionate about helping students
                                like you achieve their goals and explore the world through overseas education. Euro
                                Global Consultants has the knowledge, experience, and resources to help you make the
                                most of your study abroad experience. Contact us today to learn more about how we can
                                help you achieve your dreams.
                            </p>
                        </div>
                    </div><!-- /.col- -->
                </div> <!-- /.row -->
            </div> <!-- /.container -->
        </section> <!-- /.service-details -->

        <section class="training-section course_section">
            <div class="auto-container">
                <h3 class="title1">Find Your Dream Program</h3>
                <div class="filters">

                    <div class="ui-group">
                        <h3>Find by Level of Study</h3>
                        <div class="button-group js-radio-button-group" data-filter-group="color1">
                            <!-- <button class="button is-checked" data-filter="">All</button> -->
                            <button class="button is-checked" data-filter=".bachelors">Bachelors</button>
                            <button class="button" data-filter=".masters">Masters</button>

                        </div>
                    </div>

                    <div class="ui-group">
                        <h3>Find by Country</h3>
                        <div class="button-group js-radio-button-group" data-filter-group="size">
                            <button class="button is-checked" data-filter="*">All</button>
                            <!-- <button class="button" data-filter=".usa">USA</button>
                            <button class="button" data-filter=".uk">UK</button>
                            <button class="button" data-filter=".europe">Europe</button>
                            <button class="button" data-filter=".canada">Canada</button>
                            <button class="button" data-filter=".germany">Germany</button> -->
                            <button class="button" data-filter=".italy">Italy</button>
                            <button class="button" data-filter=".hungary">Hungary</button>
                            <button class="button" data-filter=".latvia">Latvia</button>
                            <button class="button" data-filter=".slovakia">Slovakia</button>
                            <button class="button" data-filter=".portugal">Portugal</button>
                            <button class="button" data-filter=".czech">Czech Republic</button>
                            <button class="button" data-filter=".lithuania">Lithuania</button>
                            <button class="button" data-filter=".malta">Malta</button>
                            <!-- <button class="button" data-filter="."></button> -->


                        </div>
                    </div>
                    <div class="ui-group">
                        <h3>Find by Domain</h3>
                        <div class="button-group js-radio-button-group" data-filter-group="color">

                            <button class="button is-checked" data-filter=".engineering">Engineering</button>
                            <button class="button" data-filter=".arts">Arts &amp; Science</button>
                            <button class="button" data-filter=".management">Management</button>
                            <button class="button" data-filter=".health">Health &amp; Medicine</button>
                            <button class="button" data-filter=".Building">Building &amp; Architecture</button>


                        </div>
                    </div>
                </div>
                <div class="row grid mt-5">
					<!-- Service Block -->
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">
							<!-- <div class="image-box">
								<figure class="image"><a href="contact.html"><img
											src="images/about1.webp" alt=""></a></figure>
								
							</div> -->
							<div class="content-box">
								<h6 class="lab">Mechatronics</h6>
								<h5 class="title"><a href="contact.html">Electronics Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
								<!-- <div class="university">
									<h6><b class="f500">Category :</b> Mechatronics</h6>
								</div> -->

							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechatronics</h6>
								<h5 class="title"><a href="contact.html">Automation and Control Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechatronics</h6>
								<h5 class="title"><a href="contact.html">Management Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechatronics</h6>
								<h5 class="title"><a href="contact.html">Electrical Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechatronics</h6>
								<h5 class="title"><a href="contact.html">Mechanical Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechatronics</h6>
								<h5 class="title"><a href="contact.html">Electronics Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>

					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechanical</h6>
								<h5 class="title"><a href="contact.html">Mechanical Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechanical</h6>
								<h5 class="title"><a href="contact.html"> Integrated Product Design</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>

					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechanical</h6>
								<h5 class="title"><a href="contact.html">Nuclear Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>

					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechanical</h6>
								<h5 class="title"><a href="contact.html">Mechanical Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Sapienza</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>

					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechanical</h6>
								<h5 class="title"><a href="contact.html">Energy Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Computer Science and Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Computer Science - Informatics</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Sapienza</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Artificial Intelligence and Robotics</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Sapienza</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Cognitive Neuroscience</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Sapienza</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Cybersecurity</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Sapienza</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Data Science</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Sapienza</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Computer Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Computer Science</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Control Systems Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Cybersecurity</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Data Science</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">ICT for Internet and multimedia</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Electrical and Electronics</h6>
								<h5 class="title"><a href="contact.html">Electronics Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Electrical and Electronics</h6>
								<h5 class="title"><a href="contact.html">Electrical Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Electrical and Electronics</h6>
								<h5 class="title"><a href="contact.html">Telecommunications</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Electrical and Electronics</h6>
								<h5 class="title"><a href="contact.html">Automation and Control Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Electrical and Electronics</h6>
								<h5 class="title"><a href="contact.html">Communication Design</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Electrical and Electronics</h6>
								<h5 class="title"><a href="contact.html">Control Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Sapienza </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Electrical and Electronics</h6>
								<h5 class="title"><a href="contact.html">Computer Science and Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Electrical and Electronics</h6>
								<h5 class="title"><a href="contact.html">Biomedical Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Electrical and Electronics</h6>
								<h5 class="title"><a href="contact.html">Nuclear Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape bachelors italy engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Electrical and Electronics</h6>
								<h5 class="title"><a href="contact.html">Product Service System Design</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">Business Administration</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">Economics and Finance</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">Entrepreneurship and Innovation</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Civil</h6>
								<h5 class="title"><a href="contact.html">Civil Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Civil</h6>
								<h5 class="title"><a href="contact.html">Civil Engineering for Risk Mitigation</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Civil</h6>
								<h5 class="title"><a href="contact.html">Environmental and Land Planning
										Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors Building">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Civil</h6>
								<h5 class="title"><a href="contact.html">Interior and Spatial Design</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Civil</h6>
								<h5 class="title"><a href="contact.html">Urban Planning and Policy Design</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Civil</h6>
								<h5 class="title"><a href="contact.html">Energy Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Civil</h6>
								<h5 class="title"><a href="contact.html">Environmental Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Civil</h6>
								<h5 class="title"><a href="contact.html">Water and Geological Risk Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biotechnology</h6>
								<h5 class="title"><a href="contact.html">Biological Sciences</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Unicam</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biotechnology</h6>
								<h5 class="title"><a href="contact.html">Biotechnologies for food science</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biotechnology</h6>
								<h5 class="title"><a href="contact.html">Environmental Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biotechnology</h6>
								<h5 class="title"><a href="contact.html">Food and Health</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biotechnology</h6>
								<h5 class="title"><a href="contact.html">Medical Biotechnologies</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biotechnology</h6>
								<h5 class="title"><a href="contact.html">Pharmaceutical Biotechnologies</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biomedical</h6>
								<h5 class="title"><a href="contact.html">Biomedical Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biomedical</h6>
								<h5 class="title"><a href="contact.html">Electronics Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biomedical</h6>
								<h5 class="title"><a href="contact.html">Electrical Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biomedical</h6>
								<h5 class="title"><a href="contact.html">Automation and Control Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors Building">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Architecture</h6>
								<h5 class="title"><a href="contact.html">Architecture-Architectural Design &
										History</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors Building">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Architecture</h6>
								<h5 class="title"><a href="contact.html">Architecture-Building Architecture</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors Building">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Architecture</h6>
								<h5 class="title"><a href="contact.html">Architecture-Built Environment
										Interiors</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors Building">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Architecture</h6>
								<h5 class="title"><a href="contact.html">Architecture and Urban Design</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors Building">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Architecture</h6>
								<h5 class="title"><a href="contact.html">Building and Architectural Engineeirng</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors Building">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Architecture</h6>
								<h5 class="title"><a href="contact.html">Interior and Spatial Design</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors Building">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Architecture</h6>
								<h5 class="title"><a href="contact.html">Sustainable Architecture amd Landscape
										Design</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Chemical</h6>
								<h5 class="title"><a href="contact.html">Energy Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Chemical</h6>
								<h5 class="title"><a href="contact.html">Environmental Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Chemical</h6>
								<h5 class="title"><a href="contact.html">Sustainable Science and Technology for
										Circular Economy</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Aeronautical</h6>
								<h5 class="title"><a href="contact.html">Aeronautical Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Aeronautical</h6>
								<h5 class="title"><a href="contact.html">Space Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Environmental and Energy</h6>
								<h5 class="title"><a href="contact.html">Environmental and Land Planning
										Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Environmental and Energy</h6>
								<h5 class="title"><a href="contact.html">Sustainable Agriculture</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Environmental and Energy</h6>
								<h5 class="title"><a href="contact.html">Water and Geological Risk Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Nanotechnology</h6>
								<h5 class="title"><a href="contact.html">Materials Engineering and
										Nanotechnology</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Nanotechnology</h6>
								<h5 class="title"><a href="contact.html">Sustainable Science and Technology for
										Circular Economy</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Geoinformatics</h6>
								<h5 class="title"><a href="contact.html">Geoinformatics</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Geoinformatics</h6>
								<h5 class="title"><a href="contact.html">Communication Strategies</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>

					<div class="service-block col-lg-4 color-shape italy bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Geoinformatics</h6>
								<h5 class="title"><a href="contact.html">English Studies</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>

					<div class="service-block col-lg-4 color-shape italy bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Geoinformatics</h6>
								<h5 class="title"><a href="contact.html">Forest Science</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>

					<div class="service-block col-lg-4 color-shape italy bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Physics</h6>
								<h5 class="title"><a href="contact.html">Physics</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Unicam</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Physics</h6>
								<h5 class="title"><a href="contact.html">Astrophysics and Cosmology</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Chemistry</h6>
								<h5 class="title"><a href="contact.html">Chemistry & Advanced Chemical
										Methodologies</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Unicam</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mathematics</h6>
								<h5 class="title"><a href="contact.html">Mathematics & Applications</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Unicam</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mathematics</h6>
								<h5 class="title"><a href="contact.html">Financial Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mathematics</h6>
								<h5 class="title"><a href="contact.html">Mathematical Modelling for Engineering and
										Science</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biology</h6>
								<h5 class="title"><a href="contact.html">Biological Sciences</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Unicam</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biology</h6>
								<h5 class="title"><a href="contact.html">Marine Biology</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>


					<div class="service-block col-lg-4 color-shape italy bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biology</h6>
								<h5 class="title"><a href="contact.html">Molecular Biology</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>


					<div class="service-block col-lg-4 color-shape italy bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Psychology</h6>
								<h5 class="title"><a href="contact.html">Clinical, Social and Intercultural
										Psychology</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Psychology</h6>
								<h5 class="title"><a href="contact.html">Cognitive Neuroscience and Clinical
										Neuropsychology</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Psychology</h6>
								<h5 class="title"><a href="contact.html">Developmental and Educational
										Psychology</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors health">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Food Related Courses</h6>
								<h5 class="title"><a href="contact.html">Biological Sciences</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Unicam </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors health">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Food Related Courses</h6>
								<h5 class="title"><a href="contact.html">Food Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Milano</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors health">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Food Related Courses</h6>
								<h5 class="title"><a href="contact.html">Biotechnologies for food science</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors health">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Food Engineering</h6>
								<h5 class="title"><a href="contact.html">Food and Health</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape italy bachelors health">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Food Engineering</h6>
								<h5 class="title"><a href="contact.html">Italian Food and Wine</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>University of Padova </p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape hungary bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Computer Science Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>PECS</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape hungary bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Computer Science </a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Eotvos Lorand</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape hungary bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Software & Service Architecture</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Eotvos Lorand</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape hungary bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Artificial Intelligence</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Eotvos Lorand</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape hungary bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Cyber Security</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Eotvos Lorand</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape hungary bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Data Science</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Eotvos Lorand</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape hungary bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Computer science for Autonomous
										systems</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Eotvos Lorand</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape hungary bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Civil</h6>
								<h5 class="title"><a href="contact.html">Civil Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>PECS</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape hungary bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Civil</h6>
								<h5 class="title"><a href="contact.html">Structural Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>PECS</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape hungary bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biomedical</h6>
								<h5 class="title"><a href="contact.html">Biomedical Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>PECS</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>

					<div class="service-block col-lg-4 color-shape hungary bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Film Studies</h6>
								<h5 class="title"><a href="contact.html">Film Studies</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Eotvos Lorand</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape hungary bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Psychology</h6>
								<h5 class="title"><a href="contact.html">Psychology</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>PECS</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape hungary masters Building">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Interior Desing</h6>
								<h5 class="title"><a href="contact.html">MSc in Interior and Spatial design</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>PECS</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Computer systems</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Business Informatics</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Cybersecurity Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechanical</h6>
								<h5 class="title"><a href="contact.html">Mechanical Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechanical</h6>
								<h5 class="title"><a href="contact.html">Industrial Engineering and Management</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechanical</h6>
								<h5 class="title"><a href="contact.html">Logistics and Supply Chain Management</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechanical</h6>
								<h5 class="title"><a href="contact.html">Production Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechanical</h6>
								<h5 class="title"><a href="contact.html">Design Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechatronics</h6>
								<h5 class="title"><a href="contact.html">Computerised Control of Electrical
										Technologies</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechatronics</h6>
								<h5 class="title"><a href="contact.html">Smart Electronic Systems</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechatronics</h6>
								<h5 class="title"><a href="contact.html">Industrial Engineering And management</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Electrical and Electronics</h6>
								<h5 class="title"><a href="contact.html">Telecommunications</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Electrical and Electronics</h6>
								<h5 class="title"><a href="contact.html">Smart Electronic Systems</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Electrical and Electronics</h6>
								<h5 class="title"><a href="contact.html">Railway Electrical Systems</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Electrical and Electronics</h6>
								<h5 class="title"><a href="contact.html">Computerised Control of Electrical
										Technologies</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Automobile</h6>
								<h5 class="title"><a href="contact.html">Engineering Technology</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Automobile</h6>
								<h5 class="title"><a href="contact.html">Industrial Engineering and Management</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Automobile</h6>
								<h5 class="title"><a href="contact.html">Logistics and Supply Chain Management</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Automobile</h6>
								<h5 class="title"><a href="contact.html">Production Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">Business Finance</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">Business Informatics</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">Entrepreneurship and Management</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">Logistics and Supply Chain Management</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">Innovation and Entrepreneurship</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">Organization & Management of International
										Economic Relations</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">Master's degree in Business
										Administration</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Turiba</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors Building">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Architecture</h6>
								<h5 class="title"><a href="contact.html">Construction and Real Estate
										Management</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors health">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biomedical</h6>
								<h5 class="title"><a href="contact.html">Smart Electronic Systems</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors health">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biomedical</h6>
								<h5 class="title"><a href="contact.html">Computerised Control of Electrical
										Technologies</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Chemical</h6>
								<h5 class="title"><a href="contact.html">Chemistry & Chemical Technology</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Aero</h6>
								<h5 class="title"><a href="contact.html">Aviation Transport</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Nanotechnology</h6>
								<h5 class="title"><a href="contact.html">Material Science and Nanotechnologies</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape latvia masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Technology Management</h6>
								<h5 class="title"><a href="contact.html">Industrial Engineering and Management</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>RTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape slovakia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Informatics</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>TUKE</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape slovakia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechanical</h6>
								<h5 class="title"><a href="contact.html">Mechanical Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>TUKE</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape slovakia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechanical</h6>
								<h5 class="title"><a href="contact.html">Automotive Production Technology</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>TUKE</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape slovakia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechanical</h6>
								<h5 class="title"><a href="contact.html">Industrial Mechatronics</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>TUKE</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape slovakia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechatronics</h6>
								<h5 class="title"><a href="contact.html">Industrial Mechatronics</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>TUKE</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape slovakia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechatronics</h6>
								<h5 class="title"><a href="contact.html">Automotive Production</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>TUKE</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>

					<div class="service-block col-lg-4 color-shape slovakia masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Technology Management</h6>
								<h5 class="title"><a href="contact.html">Automotive Production</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>TUKE</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape slovakia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Electrical and Electronics</h6>
								<h5 class="title"><a href="contact.html">Electrical Systems</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>TUKE</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape slovakia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biotechnology</h6>
								<h5 class="title"><a href="contact.html">Biotechnology</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>St. Cyril and Methodius</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape slovakia bachelors Building">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Architecture</h6>
								<h5 class="title"><a href="contact.html">Architecture</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>STU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape slovakia bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Aero</h6>
								<h5 class="title"><a href="contact.html">Aeronautics</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>TUKE</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape portugal bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Computer Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>IPL</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape portugal bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechanical</h6>
								<h5 class="title"><a href="contact.html">Product Design Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>IPL</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape portugal bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Electrical and Electronics</h6>
								<h5 class="title"><a href="contact.html">Electrical & Electronic Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>IPL</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape portugal bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Civil</h6>
								<h5 class="title"><a href="contact.html">Civil Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>IPL</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape portugal bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biotechnology</h6>
								<h5 class="title"><a href="contact.html">Applied Biotechnology</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>IPL</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape portugal masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">International Business </a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>IPL</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape portugal masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">Sustainable Tourism Management</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>IPL</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape portugal bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Film Studies</h6>
								<h5 class="title"><a href="contact.html">Graphic Design</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>IPL</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape czech bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechatronics</h6>
								<h5 class="title"><a href="contact.html">Mechatronics</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>TUL</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape czech bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Electrical and Electronics</h6>
								<h5 class="title"><a href="contact.html">Microelectronics</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Brno</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape czech masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">International Business and Management</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Brno</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape czech masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">Masters in Business Economics and Management</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Silesian</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape czech masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">Masters in Business Administration</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>TUL</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape czech masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">Masters in International Management</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>TUL</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape czech bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biomedical</h6>
								<h5 class="title"><a href="contact.html">Bioengineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Brno</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape czech bachelors Building">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Architecture</h6>
								<h5 class="title"><a href="contact.html">Architecture</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Brno</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>

					<div class="service-block col-lg-4 color-shape czech bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Aero</h6>
								<h5 class="title"><a href="contact.html">Aerospace Technology</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>Brno</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>

					<div class="service-block col-lg-4 color-shape czech bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Textile Engineering</h6>
								<h5 class="title"><a href="contact.html">Textile Engineering</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>TUL</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>

					<div class="service-block col-lg-4 color-shape lithuania bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">IT and CSE</h6>
								<h5 class="title"><a href="contact.html">Applied Informatics</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>VDU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape lithuania masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">International Business</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>KTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape lithuania masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">Accounting and Auditing</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>KTU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape lithuania masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">International Marketing Management</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>ISM</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape lithuania masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">Innovation and Technology Management</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>ISM</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape lithuania masters management">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Management</h6>
								<h5 class="title"><a href="contact.html">Financial Economics</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>ISM</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape lithuania bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Environmental and Energy</h6>
								<h5 class="title"><a href="contact.html">Environmental Management</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>VDU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape lithuania bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biotechnology</h6>
								<h5 class="title"><a href="contact.html">Ms in Molecular Biology and Biotechnology</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>VDU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape lithuania bachelors arts">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Biotechnology</h6>
								<h5 class="title"><a href="contact.html">Applied Biotechnology</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>VDU</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
					<div class="service-block col-lg-4 color-shape malta bachelors engineering">
						<div class="inner-box">

							<div class="content-box">
								<h6 class="lab">Mechanical </h6>
								<h5 class="title"><a href="contact.html">Mechanical Engineering and Sustainable Technology</a>
								</h5>
								<div class="row">

									<div class="col-md-12">
										<div class="course_tags">
											<div class="course_tag">
												<div class="icontag">
													<i class="fa fa-university"></i>
												</div>
												<div class="icon_text">
													<h6>University</h6>
													<p>MCAST</p>
												</div>
											</div>
										</div>
									</div>
								</div>


								<div class="tags py-2">
									<span class="cc1 cc">NO-IELTS</span>
									<span class="cc3 cc">2-Year Stayback</span>
									<span class="cc2 cc"><a style="color: inherit;" href="contact.html">Know More</a></span>
								</div>
							</div>
						</div>

					</div>
				




				</div>
            </div>
        </section>

		<section class="faq_section">
			<div class="container">
				<div class="row">
					<div class="col">
						<h3 class="title1">Frequently Asked Questions</h3>
						<div class="accordion-one">
							<div class="panel-group theme-accordion" id="accordion">
								<div class="panel">
									<div class="panel-heading active-panel">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse1">
												Why study abroad?</a>
										</h6>
									</div>
									<div id="collapse1" class="panel-collapse collapse show">
										<div class="panel-body">
											<p>
												For most international students, it is likely to be the combination of quality education, cultural immersion, the world view, and future job opportunities. It results in enhanced employment prospects and increased awareness of how the world functions.
											</p>
										</div>
									</div>
								</div> <!-- /panel 1 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse2">
												How do I get started?</a>
										</h6>
									</div>
									<div id="collapse2" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												To make things much easier for you, we will be sorting out everything right from admission to Visa. The only thing you need to do is come and meet our team of Counselors at one of our branches. When choosing your university, they will provide you with a large amount of information concerning studying in Europe and the courses that might suit your interests and academic profile. The section on choosing a program and a university provides information about the selection of the program and university, the requirements for admission and the costs, the Processes for applying, Scholarships, the Possibility of working after completing a course, a Visa, and so on.
											</p>
										</div>
									</div>
								</div> <!-- /panel 2 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse3">
												How much does it cost to study abroad?</a>
										</h6>
									</div>
									<div id="collapse3" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												There are significant costs that need to be catered for as part of studying abroad and so the gross cost of studying in different countries of your interest can only be determined if you add the average international tuition and other expenses such as those of living. A few years ago, some countries even had no tuition fees at all. Germany and Norway countries can be taken as examples of such countries. Taris for such things as accommodation, food, and drinks to transport fare, etc. differ across Europe. The overall living expenses might range between Rs. 20,000 and Rs. 50,000 depending on the Countries you decide to residency.
											</p>
										</div>
									</div>
								</div> <!-- /panel 3 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse4">
												When can I start applying for study abroad programs?</a>
										</h6>
									</div>
									<div id="collapse4" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												With regard to your application, you should do it as early as possible. This is the best approach to take. After all, the sooner the acceptance to a university is gained, the sooner one can plan the trip. Deadlines may vary based on the school or university but for programs starting in September/October; applications may be accepted from January/February up to mid-June/July.
											</p>
										</div>
									</div>
								</div> <!-- /panel 4 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse5">
												What are the entry requirements for study abroad programs?</a>
										</h6>
									</div>
									<div id="collapse5" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												The entry requirements differ from one university to the other as well as by country We shall assist you to present the documents that the Universities of your choice require. 
												In general, however, if you are applying as an undergraduate, you will be required to produce proof of your secondary education to the standard set for the program for which you’re applying (for instance your GPA, A levels, or any other equivalent qualifications). 
												Finally, for non-native English speakers wishing to study in English-speaking countries, it is also likely that you will be required to present certification of your English competency through a test, probably TOEFL or IELTS. Other related tests may be necessary for those studying in other languages

											</p>
										</div>
									</div>
								</div> <!-- /panel 5 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse6">
												What documentation should I submit with my application?
											</a>
										</h6>
									</div>
									<div id="collapse6" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												There might be some documents required to be submitted by you as part of your application. Documents that are required to be submitted might differ from country to country and university. Here are some of the common documents required from international students:

												<ul class="list-faq">
													<li>A statement of purpose</li>
													<li>CV/résumé</li>
													<li>Academic references/ letters of recommendation</li>
													<li>Official academic records of your secondary education or certificate of Equivalency. </li>
													<li>Original certificates that test the applicant’s qualification in the English language ( TOEFL/IELTS certificate), particularly for schools in English-speaking countries, or any other language test.</li>
													<li>Admissions test results (e.g. GMAT/GRE results, for graduate programs)</li>
												</ul>
											</p>
										</div>
									</div>
								</div> <!-- /panel 6 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse7">
												Do I have to speak a second language to study abroad?
											</a>
										</h6>
									</div>
									<div id="collapse7" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												It depends on the country in which you intend to study and the language the course will be taught in. If a student is however not a native English speaker and wants to follow a course taught in English, he/she must show English language test results to assure the university he/she is good at the language. This is to avoid a situation whereby you are following a particular course, and you seem to have a lot of problems grasping what is being taught. 
												<br><br>
												English acts as the medium for instruction in many other countries of the world, especially for master’s and doctorate courses and business administration degrees. As for the tests, the TOEFL and IELTS are the most widely recognized and authoritative tests to prove one’s knowledge of English. In case you need to demonstrate your proficiency in a language other than English there are similar tests in other languages for instance DELF/DALF, TCF-DAP (French), and DSF or TestDaF (German).
	
											</p>
										</div>
									</div>
								</div> <!-- Panel 7 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse8">
												Will I need to attend an admissions interview?
											</a>
										</h6>
									</div>
									<div id="collapse8" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												Some universities conduct their interviews in different parts of the world so there might be a chance that you will be asked to be a part of one of those. There is also an increasing focus on video interviews as well. This will be like any other interview arranged on a certain time and date on the calendar though it will be a virtual interview through the use of an app such as Skype.
											</p>
										</div>
									</div>
								</div> <!-- Panel 8 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse9">
												After gaining a letter of acceptance, what do I do next?
											</a>
										</h6>
									</div>
									<div id="collapse9" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												Congratulations, you’re in! All that is left to do now is study for your exams, pack all your life into one very large suitcase, apply for your travel visas, discover where you will be living, and find the funds required… and it will all be worth it! 
												<br><br>
												Once you have secured admission to a university, one of the first things that you should begin to contemplate is travel documents. Check that you have a passport that is not expired, necessary travel insurance, and a student visa if you are going to need it. Ensure that you have enough time to get your passport/visa processed so that you’ll be able to legally leave for the set destination. We will help you to compile the necessary documents that will enable you to get the student visa. 

											</p>
										</div>
									</div>
								</div> <!-- Panel 9 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse10">
												Is a student visa required for entry? 
											</a>
										</h6>
									</div>
									<div id="collapse10" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												If you are from another country/continent, then most likely you will have to get a student visa to enter the selected country. This is often only relevant where the stay is for a longer period of International Study; if your study exchange will be less than three months, a Tourist/Visitor Visa might suffice.
											</p>
										</div>
									</div>
								</div> <!-- Panel 10 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse11">
												Can I get financial aid to study abroad?
											</a>
										</h6>
									</div>
									<div id="collapse11" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												While most international students face a lot of challenges accessing student loans to finance their education, numerous other sources of funding make studying abroad possible hence some of the known funding facilities include scholarships, fellowships, studentships, sponsorships, grants, and bursaries. There are commonly two types of scholarships – merit-based scholarships and other scholarships; the former ones are usually very competitive.
											</p>
										</div>
									</div>
								</div> <!-- Panel 11 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse12">
												What kind of accommodation will be available to me during the time that I will be studying abroad? 
											</a>
										</h6>
									</div>
									<div id="collapse12" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												If the university of your preference has available and accessible university-managed halls of residence, it will probably be possible to apply for a place there. If you do not meet this requirement, you will be required to seek your accommodation arrangements. <br><br>
												If finance is not a problem, you could rent a flat separately if you don’t have so much money but want to rent a room separately you can find other students ready to share an apartment or use the “spare room” offers that can be seen on the Internet. It is also your university’s student support team and the student union that should guide the local means of finding tenancy in a property.
											</p>
										</div>
									</div>
								</div> <!-- Panel 12 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse13">
												Can I work during my studies abroad? 
											</a>
										</h6>
									</div>
									<div id="collapse13" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												This will depend on your student visa policy whereby you may find that you are allowed to work or not. Some countries have laws that regulate the amount of paid work a student is allowed to do while attending classes. Sometimes it is allowed no more than 20 hours of paid work within the week during term time, but during holidays, you can engage in full-time work.
											</p>
										</div>
									</div>
								</div> <!-- Panel 13 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse14">
												How long will my degree course take if I study in Europe?
											</a>
										</h6>
									</div>
									<div id="collapse14" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												The average length for degree programmes is:
												<ul class="list-faq">
													<li>Bachelor's degree programme: 3 years / 4 Years</li>
													<li>Master's degree programme: 1 / 2 years</li>
												</ul>
											</p>
										</div>
									</div>
								</div> <!-- Panel 14 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse15">
												Is it also possible to study in English?
											</a>
										</h6>
									</div>
									<div id="collapse15" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												In addition to study programmes in their official languages, most European countries also offer many programmes taught in English.
											</p>
										</div>
									</div>
								</div> <!-- Panel 15 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse16">
												Will European universities accept my school-leaving certificate? Or the fact that I previously earned a degree in another country? Or that I have already studied in my home country for some time? 
											</a>
										</h6>
									</div>
									<div id="collapse16" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												This depends on each country and in most cases, on each higher education institution tending to the students. We will get in touch with the International Office of the European university of your choice to see whether your certificate/degree which you acquired already, will be accredited.
											</p>
										</div>
									</div>
								</div> <!-- Panel 16 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse17">
												How can I find the best university?
											</a>
										</h6>
									</div>
									<div id="collapse17" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												Given the fact that there are thousands and thousands of universities and other institutions of higher learning in Europe, it poses an endless question as to which one is the best university for you. Every learner has his or her concerns, preferences, and desires. Our recommendation is to come to one of our branches and sit down with our Counselors so that the process of filtering out the Universities of your interest turns out to be easier.
											</p>
										</div>
									</div>
								</div> <!-- Panel 17 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse18">
												Do I have the option to stay within the country to work after my studies in Europe?
											</a>
										</h6>
									</div>
									<div id="collapse18" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												Every country in Europe has its policies on how it allows students to linger in the country after their courses in search of a job. However, according to the European Union, international students are required to look for a work opportunity for at least 9 months.
											</p>
										</div>
									</div>
								</div> <!-- Panel 18 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse19">
												What does a master's degree mean?
											</a>
										</h6>
									</div>
									<div id="collapse19" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												A master’s degree is an advanced university degree which is pursued after the fruitful completion of an undergraduate bachelor's degree. The two broadly typical forms of the Master are the MA (Master of Arts) & the MSc (Master of Science) degrees.
											</p>
										</div>
									</div>
								</div> <!-- Panel 19 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse20">
												What are the differences between a master’s and a PhD degree? 
											</a>
										</h6>
									</div>
									<div id="collapse20" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												A master's and a PhD are awards of the same level of study, but a PhD is higher than a master's degree. Master's degrees focus more on current subject knowledge than on researching new information. PhD study concentrates on new research that introduces new information or knowledge into the area of specialization.
											</p>
										</div>
									</div>
								</div> <!-- Panel 20 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse21">
												Is post-graduate study the same as graduate study?
											</a>
										</h6>
									</div>
									<div id="collapse21" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												The terms graduate and post-graduate are similar terms that are used to describe university degrees <br>
												In the UK, postgraduate study is understood as courses that start after receiving a bachelor's degree (a post-graduate degree). <br>
												Graduate study is a North American term for courses that start once someone is a graduate (after the completion of a bachelor's degree). <br>
												Therefore, fundamentally, a master’s degree in the UK is called a postgraduate degree while a master's degree in the USA is called a graduate degree.
											</p>
										</div>
									</div>
								</div> <!-- Panel 21 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse22">
												What am I in terms of education, if I hold a master’s degree? 
											</a>
										</h6>
									</div>
									<div id="collapse22" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												A master’s degree is a second-cycle award, which comes after a bachelor’s degree but before a PhD or any other doctoral degree. Most individuals pursue a master’s as a postgraduate, which means they complete a first degree before undertaking a master's degree.
											</p>
										</div>
									</div>
								</div> <!-- Panel 22 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse23">
												How many credits is a Masters?
											</a>
										</h6>
									</div>
									<div id="collapse23" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												A UK Masters degree is worth 180 credits. Elsewhere in Europe a Masters is worth 120 / 90 or 60 ECTS credits.
											</p>
										</div>
									</div>
								</div> <!-- Panel 23 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse24">
												How difficult is a master’s degree?
											</a>
										</h6>
									</div>
									<div id="collapse24" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												In master’s degrees, students get to learn more sophisticated work than in bachelor’s degrees and the student is supposed to study on their own most of the time. However, it is generally expected for anyone who has already obtained an undergraduate degree to be ready for postgraduate studies.
											</p>
										</div>
									</div>
								</div> <!-- Panel 24 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse25">
												How many hours does one require to dedicate to a master’s degree program? 
											</a>
										</h6>
									</div>
									<div id="collapse25" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												This varies in different courses, but as a postgraduate student, one will require more self-studying to excel in their studies. You will need to expect to read for a minimum of 20-25 hours a week excluding your schedule of classes and lectures.
											</p>
										</div>
									</div>
								</div> <!-- Panel 25 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse26">
												Are you required to write a dissertation for a master’s program? 
											</a>
										</h6>
									</div>
									<div id="collapse26" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												Yes. Undergraduate degrees do not have such requirements while all master's Degrees are completed with a dissertation, thesis, or something that could be a little similar. This normally covers the third semester of your course and a part of the summer as well. If no dissertation is included, then, probably the course is at the Postgraduate Diploma or Certificate level.
											</p>
										</div>
									</div>
								</div> <!-- Panel 26 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse27">
												Do master’s Degrees involve taking examinations?
											</a>
										</h6>
									</div>
									<div id="collapse27" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												Master’s Degrees assessments also differ according to the areas of study. As for the Practical Tasks, Courses in Arts and Humanities shall be usually completed with essays and coursework. STEM subjects may also require the students to write a formal examination to pass an assessment of the major concepts and practices.
											</p>
										</div>
									</div>
								</div> <!-- Panel 27 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse28">
												Is it possible to work while studying for a Masters? 
											</a>
										</h6>
									</div>
									<div id="collapse28" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												It is possible to work at least part-time while studying for a master’s for some students. The limited contact time for some courses can make it easier to balance work and study, but you should resist the temptation to take on too many working hours: a master’s does consequently entail more private studying time than a Bachelor's. International students should also ensure that working hours are not limited by visa type for them to allow ourselves a better working experience.
											</p>
										</div>
									</div>
								</div> <!-- Panel 28 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse29">
												What's the difference between an MA and an MSc?
											</a>
										</h6>
									</div>
									<div id="collapse29" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												An MA (Master of Arts) is usually offered in Arts, Humanities and certain social sciences disciplines. The MSc (Master of Science) degree is normally given in science, technology Engineering, Medicine, and Mathematics disciplines. They both mean the same, last for the same time and entail the same kind of studies, it is only the subject of the degree that varies.
											</p>
										</div>
									</div>
								</div> <!-- Panel 29 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse30">
												Is an MBA a Masters?
											</a>
										</h6>
									</div>
									<div id="collapse30" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												MBA (Master of Business Administration) is a very popular professional master’s degree that is conceived for Business Management experts. <br>
												Generally, MBAs take more time than most master’s and they are mostly taken part-time. Other Business postgraduate degrees like MIM (master’s in management) are for someone coming from an undergraduate program.

											</p>
										</div>
									</div>
								</div> <!-- Panel 30 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse31">
												Which are the most chosen fields of study for a master’s degree? 
											</a>
										</h6>
									</div>
									<div id="collapse31" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												The most popular postgraduate subjects in Europe are STEM courses, business studies and business-related programs, teacher training, economics and biological sciences. Fortunately, there is such a massive selection of master's Degrees available so spend the time and get the perfect one for you.
											</p>
										</div>
									</div>
								</div> <!-- Panel 31 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse32">
												Are rankings relevant while pursuing a master’s degree?  
											</a>
										</h6>
									</div>
									<div id="collapse32" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												The overall university rankings are integrated from measures that do not reveal how valuable or competitive the master’s Courses in a university are. When using rankings for postgraduate study, one must ‘zoom in,’ to specific areas with specific metrics in the list or consider subject-wise classifications.
											</p>
										</div>
									</div>
								</div> <!-- Panel 32 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse33">
												What is the best subject to study for a master’s degree?   
											</a>
										</h6>
									</div>
									<div id="collapse33" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												Different goals require different postgraduate subjects, so the best subject to study would highly depend on your goals in doing a master's degree. Depending on the chosen program of the master's degree, some of them provide direct preparation for certain employment, but most academic master's degrees can also be an opportunity to study more of the subject, interested in, to develop more independent work, critical thinking and handling more challenging tasks.
											</p>
										</div>
									</div>
								</div> <!-- Panel 33 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse34">
												How can one select a master’s degree?    
											</a>
										</h6>
									</div>
									<div id="collapse34" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												There are lots of factors to consider when choosing a master’s degree:
												<ul class="list-faq">
													<li>Relevance – simply, is your course of the type you want: on the right subject, in the right style, from the right perspective? Have the university chosen the modules and dissertation topics that would be properly interesting? </li>
													<li>Reliability and ranking of the University – is the University well-known for your course/field of study? </li>
													<li>Tuition fee- While the cost of tuition fee for master’s degrees differs from one institution to another, some universities charge a lot more than others.</li>
												</ul>
											</p>
										</div>
									</div>
								</div> <!-- Panel 34 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse35">
												What kind of credits or grades are required for a master’s program?    
											</a>
										</h6>
									</div>
									<div id="collapse35" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												Typically, you will require a prior degree at least equivalent to a bachelor’s degree to gain entry to a master's degree. A 2.1 or higher is the standard entry requirement, being 60% marks on average It is however possible to do a master’s with a lower-class degree.
											</p>
										</div>
									</div>
								</div> <!-- Panel 35 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse36">
												What is the expected GPA of a master's student?    
											</a>
										</h6>
									</div>
									<div id="collapse36" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												Some universities in the USA will factor in your undergraduate GPA score as a requirement when applying for a master’s degree. A GPA of 3.0 or above is normally ‘good’ – though it translates to roughly an equivalent of a UK 2. 1. The GPA system is not commonly implemented in most of the Universities in the UK and Europe.
											</p>
										</div>
									</div>
								</div> <!-- Panel 36 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse37">
												Is it possible to study for a master’s degree without a bachelor’s degree?   
											</a>
										</h6>
									</div>
									<div id="collapse37" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												Most universities requiring for master’s study insist that the applicant must be a holder of an undergraduate degree in the same field.
											</p>
										</div>
									</div>
								</div> <!-- Panel 37 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse38">
												Is IELTS compulsory for a master’s degree?   
											</a>
										</h6>
									</div>
									<div id="collapse38" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												If you are planning to apply for a master’s course taught in English, in a country other than your own and if English is not your first language, then you may be required to take an IELTS for postgraduate courses test. In this case, exceptions may be made under circumstances where the person in question is an international student who has done another degree which is administered in English. Some of the other commonly used tests include TOEFL.
											</p>
										</div>
									</div>
								</div> <!-- Panel 38 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse39">
												Is GMAT required for a master’s program?  
											</a>
										</h6>
									</div>
									<div id="collapse39" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												The Graduate Management Admissions Test, or GMAT, is a graduation-level examination test that is applied for admission for some of the MBA programs and other business master’s levels. It determines if you have the right critical thinking skills and management knowledge to excel in these types of courses. 
												<br><br>
												Once again, the GMAT exams are commonly included as part of applications toward even more selective Business Schools in the UK and USA; however, they are not obligatory for all universities.

											</p>
										</div>
									</div>
								</div> <!-- Panel 39 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse40">
												Is a GRE necessary for a master’s degree? 
											</a>
										</h6>
									</div>
									<div id="collapse40" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												The GRE stands for Graduate Record Examination and is a graduate admission test applicable to almost all subjects excluding business and management in which GMAT is used. It challenges your critical assessment, reading and writing skills and even your numerical skills to determine how fit you are for post-graduate education and training. 
												<br><br>
												The GRE is not much required in the UK and Europe but is used in some selective Universities in the USA and some other countries.
											</p>
										</div>
									</div>
								</div> <!-- Panel 40 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse41">
												What are the requirements for master’s application? 
											</a>
										</h6>
									</div>
									<div id="collapse41" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												The application for a master's student is normally up taken by the universities and most of them will require all or some of the following:
												<ul class="list-faq">
													<li>Certification of your last/anticipated university performances or a certificate of your university degree, if you are a university graduate </li>
													<li>A personal statement, about why they want to do this course </li>
													<li>Two or three references (it can include previous course tutors). </li>
													<li>Details of how you propose to finance your postgraduate studies </li>
													<li>Documents showing your proficiency in the language in which you are studying in, if you are in a non-traditional country. </li>
													<li>Graduate admittance test results (if it can be provided given that you are applying for more selective/ competitive programs). </li>
													<li>Some universities have interviews to confirm that the student is in the right course or to choose from the list of courses that admit few students.</li>
												</ul>
											</p>
										</div>
									</div>
								</div> <!-- Panel 41 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse42">
												Is it possible to do a master’s degree without paying for it? 
											</a>
										</h6>
									</div>
									<div id="collapse42" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												Some of the countries have no tuition charges for master’s Degrees. This is not entirely exempt from paying as this means that you will still have to cater for the living expenses as well as other expenses that are associated with the degree course.
											</p>
										</div>
									</div>
								</div> <!-- Panel 42 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse43">
												Is a master's degree cheaper if studied abroad?
											</a>
										</h6>
									</div>
									<div id="collapse43" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												The costs for postgraduate programs vary and some countries actually have very low fees, if any at all, so one should look at the costs in different countries. It is however important to note that in an online degree program you still have to factor the costs of rent and other living expenses in.
											</p>
										</div>
									</div>
								</div> <!-- Panel 43 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse44">
												Are all master’s degrees worth the same?
											</a>
										</h6>
									</div>
									<div id="collapse44" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												All properly accredited and awarded master's Degrees are respected academic qualifications (an MA is worth the same as an MSc, and so on).
											</p>
										</div>
									</div>
								</div> <!-- Panel 44 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse45">
												Will you earn more with a master’s degree?
											</a>
										</h6>
									</div>
									<div id="collapse45" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												The responding data proves with more credibility that the givers of a master’s degree are paid more than other recipients of a degree. However, one cannot be very sure of the fact that a master’s is going to increase your earnings in clinical practice without a doubt.
											</p>
										</div>
									</div>
								</div> <!-- Panel 45 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse46">
												Can you land a better job with a master’s? 
											</a>
										</h6>
									</div>
									<div id="collapse46" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												As evidenced, few entry jobs require a holder to possess a master’s degree in their specific field of work. Still, as far as your education is concerned, a postgraduate qualification is good because it shows that you are capable of mastering more complex information, can work to a greater extent on your own, and will have a body of knowledge within your field of study. It is also found that some subjects do produce very employable (post)graduates similarly the recommendations of APU are also acceptable in industries.
											</p>
										</div>
									</div>
								</div> <!-- Panel 46 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse47">
												Is it necessary to have a master’s degree to be able to carry out a PhD? 
											</a>
										</h6>
									</div>
									<div id="collapse47" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												One doesn't need to hold a master’s degree before applying for a PhD though it is wise to. Arts and Humanities subjects, for example, expect the candidate to have had some taste of postgraduate work before going for the doctorate. STEM subjects are more likely to accept applications directly from the undergraduate level while getting a master’s degree may help you win a funded spot.
											</p>
										</div>
									</div>
								</div> <!-- Panel 47 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse48">
												Can a master’s degree make you overqualified? 
											</a>
										</h6>
									</div>
									<div id="collapse48" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												Postgraduate certification will usually be appreciated (even if as a rule the company doesn’t look for such qualifications) and your subject of study at master’s doesn’t exactly have to correspond to the job you searched for afterwards. 
												<br><br>
												Nevertheless, you should always be able to justify why you decided to pursue a degree (at any level) and what you derived from the process.
											</p>
										</div>
									</div>
								</div> <!-- Panel 48 -->
								<div class="panel">
									<div class="panel-heading">
										<h6 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion" href="#collapse49">
												Is the Europe Study Centre legitimate in assisting in getting admission to universities in foreign countries? 
											</a>
										</h6>
									</div>
									<div id="collapse49" class="panel-collapse collapse">
										<div class="panel-body">
											<p>
												Yes. Given the facts, it is quite certain that the Europe Study Centre is a bona fide organization. We have been in this field for the last 13 years and have almost 29+ branches and franchises. We do profile analysis right and offer the students/clients the right options. Admission, visas and anything else will be explained very comprehensively on the first hand. The company provides exactly what it has guaranteed. The universities are also ranked among the best ones within the given European countries. Ranking very few universities have granted us exclusivity for directly recruiting Indian students. Everything from admission to visa will be arranged by us. Rest assured our services are something you can put your trust in.
											</p>
										</div>
									</div>
								</div> <!-- Panel 49 -->
							</div> <!-- end #accordion -->
						</div>
					</div>
				</div>
			</div>
		</section>

		<section class="clients-section1" style="padding: 100px 0 100px;">
			<div class="auto-container">
				<div class="sec-title text-center mb-5">
					<span class="sub-title">Universities</span>
					<h2>We Collabrate With</h2>
				</div>
				<!-- Sponsors Outer -->
				<div class="sponsors-outer my-5">
					<!--clients carousel-->
					<ul class="clients-carousel1 owl-carousel owl-theme">
						<li class="slide-item"> <img src="images/logo_client/2.webp" alt="EuroGlobal client"></li>
						<li class="slide-item"> <img src="images/logo_client/9.webp" alt="EuroGlobal client"></li>
						<li class="slide-item"> <img src="images/logo_client/12.webp" alt="EuroGlobal client"></li>
						<li class="slide-item"> <img src="images/logo_client/13.webp" alt="EuroGlobal client"></li>
						<li class="slide-item"> <img src="images/logo_client/14.webp" alt="EuroGlobal client"></li>

					</ul>
				</div>
				<div class="sponsors-outer my-5">
					<!--clients carousel-->
					<ul class="clients-carousel1 owl-carousel owl-theme">

						<li class="slide-item"> <img src="images/logo_client/15.webp" alt="EuroGlobal client"></li>
						<li class="slide-item"> <img src="images/logo_client/16.webp" alt="EuroGlobal client"></li>
						<li class="slide-item"> <img src="images/logo_client/3.webp"  alt="EuroGlobal client"></li>


						<li class="slide-item"> <img src="images/logo_client/20.webp" alt="EuroGlobal client"></li>
						<li class="slide-item"> <img src="images/logo_client/5.webp"  alt="EuroGlobal client"></li>
						<li class="slide-item"> <img src="images/logo_client/6.webp"  alt="EuroGlobal client"></li>

					</ul>

				</div>
				<!-- Sponsors Outer -->
				<div class="sponsors-outer my-5">
					<!--clients carousel-->
					<ul class="clients-carousel1 owl-carousel owl-theme">
						<li class="slide-item"> <img src="images/logo_client/10.webp" alt="EuroGlobal client"></li>
						<li class="slide-item"> <img src="images/logo_client/4.webp" alt="EuroGlobal client"></li>
						<li class="slide-item"> <img src="images/logo_client/18.webp" alt="EuroGlobal client"></li>
						<li class="slide-item"> <img src="images/logo_client/19.webp" alt="EuroGlobal client"></li>
						<li class="slide-item"> <img src="images/logo_client/17.webp" alt="EuroGlobal client"></li>
					</ul>

				</div>
			</div>
		</section>

		<section class="why-choose-us-three pt-0"></section>
			<div class="auto-container">
				<div class="banner-box-three">
					<div class="inner-box">
						<div class="row">
							<div class="col-md-8">
								<h3 class="title">So why wait?</h3>

								<div class="text" style="font-size: 20px;">Get in touch with us today and take the first
									step towards unlocking a world of opportunities!</div>

								<div class="text" style="font-size: 20px;">To enter the world of opportunity</div>

								<div class="btncolumn"><a class="theme-button-one mt-5"
										href="http://euroglobal.globalstudycentre.ae/"><span class="btn-title">Click
											here</span></a></div>
								</div>

							<div class="col-md-4">
								<figure class="image"><img alt="Study Abroad Consultants in UAE/Dubai" src="images/home/client1.webp" /></figure>

								<figure class="image"><img alt="Study Abroad Consultants in UAE/Dubai" src="images/home/client2.webp" /></figure>
							</div>
						</div>
					</div>
				</div>
			</div>
		</section>

		<section class="appointment_form">
			<div class="container">
				<div class="row">
					<div class="col">
						<h3 class="title1" style="margin-bottom: 0;">Interested? Book an appointment now!</h3>
						<iframe aria-label='Book an Appointment' frameborder="0" style="height:700px;width:99%;border:none;" src='https://forms.zohopublic.com/euroglobalconsultancyfzc/form/BookanAppointment/formperma/yantZQUFlBbxpyMaF3QoBbjS5pKnEguIwFHwqdG2rjg'></iframe>
					</div>
				</div>
			</div>
		</section>

        <!--
			=====================================================
				Footer
			=====================================================
			-->
        <footer class="theme-footer-one">
            <div class="top-footer">
                <div class="container">
                <div class="row">
                    <div class="col-xl-4 col-lg-4 col-sm-6 about-widget">
                    <img src="images/home/logo-2.webp" alt="">
                    <p>We are dedicated to helping individuals and organizations achieve their academic,
                        professional & business goals.</p>
                    </div> <!-- /.about-widget -->
                    <div class="col-xl-2 col-lg-3 col-sm-6 footer-list">
                    <h6 class="title">SERIVICES</h6>
                    <ul>
                        <li><a href="./IT-services.html">IT Services</a></li>
                        <li><a href="./training.html">Training</a></li>
                        <li><a href="./study-abroad.html">Study Abroad</a></li>
                    </ul>
                    </div> <!-- /.footer-list -->
                    <div class="col-xl-2 col-lg-3 col-sm-6 footer-list">
                    <h6 class="title">EXPLORE</h6>
                    <ul>
                        <li><a href="./about.html">About Us</a></li>
                        <li><a href="./blogs.html">Blogs</a></li>
                        <li><a href="./contact.html">Contact Us</a></li>
                    </ul>
                    </div> <!-- /.footer-list -->
                    <div class="col-xl-4 col-lg-2 col-sm-6 footer-newsletter">
                    <h6 class="title">CONTACT</h6>
                    <div>
                        <ul>
                        <li>
                            <i class="icon flaticon-placeholder"></i>
                            <p>EuroGlobal Consultancy, Royal house building,
                            Office No M08-05,Mezzanine floor, Al Wuheida Road, Hor Al Anz East, Dubai.
                            </p>
                        </li>
                        <li>
                            <i class="icon flaticon-multimedia"></i>
                            <a href="mailto:enquiry@euroglobalconsultancy.com">
                            enquiry@euroglobalconsultancy.com
                            </a>
                        </li>
                        <li>
                            <i class="icon flaticon-phone-call"></i>
                            <a href="tel:971544888923">
                            +971 54 4888923
                            </a>
                        </li>
                        </ul>
                    </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-12">
                    <h6 class="title">Accreditation & Memberships</h6>
                    <div class="f_logo">
                        <img src="images/logo/1.webp" alt="Study Abroad Consultancy in UAE/Dubai" loading="lazy">
                        <img src="images/logo/2.webp" alt="Study Abroad Consultancy in UAE/Dubai" loading="lazy">
                        <img src="images/logo/3.webp" alt="Study Abroad Consultancy in UAE/Dubai" loading="lazy">
                    </div>
                    </div>
                </div> <!-- /.row -->
                </div> <!-- /.container -->
            </div> <!-- /.top-footer -->
            <div class="bottom-footer">
                <div class="container">
                <div class="row">
                    <div class="col-md-4 col-12">
                    <p>&copy; Copyrights 2024. All Rights Reserved.</p>
                    </div>
                    <div class="col-md-4 col-12 text-center">
                    <a href="privacy-policy.html">Privacy Policy</a>
                    </div>
                    <div class="col-md-4 col-12">
                    <ul>
                        <li><a href="https://www.facebook.com/EuroGlobalConsultancy"><i class="fa fa-facebook" aria-hidden="true"></i></a></li>
                        <li><a href="https://www.instagram.com/euroglobalconsultancyfzc/"><i class="fa fa-instagram" aria-hidden="true"></i></a></li>
                        <li><a href="https://api.whatsapp.com/send?phone=971544888923"><i class="fa fa-whatsapp" aria-hidden="true"></i></a></li>
                        <li><a href="https://www.linkedin.com/company/euroglobal-consultancyuae"><i class="fa fa-linkedin-square" aria-hidden="true"></i></a></li>
                    </ul>
                    </div>
                </div>
                </div>
            </div> <!-- /.bottom-footer -->
        </footer>
        <!-- /.theme-footer -->




        <!-- Scroll Top Button -->
        <button class="scroll-top tran3s">
            <i class="fa fa-angle-up" aria-hidden="true"></i>
        </button>

        <div class="whatsapp">
            <a target="_blank" href="https://api.whatsapp.com/send?phone=971544888923">
            <svg style="pointer-events:none; display:block; height:40px; width:40px;" width="40px" height="40px" viewBox="0 0 1024 1024">
            <defs>
            <path id="htwasqicona-chat" d="M1023.941 765.153c0 5.606-.171 17.766-.508 27.159-.824 22.982-2.646 52.639-5.401 66.151-4.141 20.306-10.392 39.472-18.542 55.425-9.643 18.871-21.943 35.775-36.559 50.364-14.584 14.56-31.472 26.812-50.315 36.416-16.036 8.172-35.322 14.426-55.744 18.549-13.378 2.701-42.812 4.488-65.648 5.3-9.402.336-21.564.505-27.15.505l-504.226-.081c-5.607 0-17.765-.172-27.158-.509-22.983-.824-52.639-2.646-66.152-5.4-20.306-4.142-39.473-10.392-55.425-18.542-18.872-9.644-35.775-21.944-50.364-36.56-14.56-14.584-26.812-31.471-36.415-50.314-8.174-16.037-14.428-35.323-18.551-55.744-2.7-13.378-4.487-42.812-5.3-65.649-.334-9.401-.503-21.563-.503-27.148l.08-504.228c0-5.607.171-17.766.508-27.159.825-22.983 2.646-52.639 5.401-66.151 4.141-20.306 10.391-39.473 18.542-55.426C34.154 93.24 46.455 76.336 61.07 61.747c14.584-14.559 31.472-26.812 50.315-36.416 16.037-8.172 35.324-14.426 55.745-18.549 13.377-2.701 42.812-4.488 65.648-5.3 9.402-.335 21.565-.504 27.149-.504l504.227.081c5.608 0 17.766.171 27.159.508 22.983.825 52.638 2.646 66.152 5.401 20.305 4.141 39.472 10.391 55.425 18.542 18.871 9.643 35.774 21.944 50.363 36.559 14.559 14.584 26.812 31.471 36.415 50.315 8.174 16.037 14.428 35.323 18.551 55.744 2.7 13.378 4.486 42.812 5.3 65.649.335 9.402.504 21.564.504 27.15l-.082 504.226z"></path>
            </defs>
            <linearGradient id="htwasqiconb-chat" gradientUnits="userSpaceOnUse" x1="512.001" y1=".978" x2="512.001" y2="1025.023">
                <stop offset="0" stop-color="#61fd7d"></stop>
                <stop offset="1" stop-color="#2bb826"></stop>
            </linearGradient>
            <use xlink:href="#htwasqicona-chat" overflow="visible" fill="url(#htwasqiconb-chat)"></use>
            <g>
                <path fill="#FFF" d="M783.302 243.246c-69.329-69.387-161.529-107.619-259.763-107.658-202.402 0-367.133 164.668-367.214 367.072-.026 64.699 16.883 127.854 49.017 183.522l-52.096 190.229 194.665-51.047c53.636 29.244 114.022 44.656 175.482 44.682h.151c202.382 0 367.128-164.688 367.21-367.094.039-98.087-38.121-190.319-107.452-259.706zM523.544 808.047h-.125c-54.767-.021-108.483-14.729-155.344-42.529l-11.146-6.612-115.517 30.293 30.834-112.592-7.259-11.544c-30.552-48.579-46.688-104.729-46.664-162.379.066-168.229 136.985-305.096 305.339-305.096 81.521.031 158.154 31.811 215.779 89.482s89.342 134.332 89.312 215.859c-.066 168.243-136.984 305.118-305.209 305.118zm167.415-228.515c-9.177-4.591-54.286-26.782-62.697-29.843-8.41-3.062-14.526-4.592-20.645 4.592-6.115 9.182-23.699 29.843-29.053 35.964-5.352 6.122-10.704 6.888-19.879 2.296-9.176-4.591-38.74-14.277-73.786-45.526-27.275-24.319-45.691-54.359-51.043-63.543-5.352-9.183-.569-14.146 4.024-18.72 4.127-4.109 9.175-10.713 13.763-16.069 4.587-5.355 6.117-9.183 9.175-15.304 3.059-6.122 1.529-11.479-.765-16.07-2.293-4.591-20.644-49.739-28.29-68.104-7.447-17.886-15.013-15.466-20.645-15.747-5.346-.266-11.469-.322-17.585-.322s-16.057 2.295-24.467 11.478-32.113 31.374-32.113 76.521c0 45.147 32.877 88.764 37.465 94.885 4.588 6.122 64.699 98.771 156.741 138.502 21.892 9.45 38.982 15.094 52.308 19.322 21.98 6.979 41.982 5.995 57.793 3.634 17.628-2.633 54.284-22.189 61.932-43.615 7.646-21.427 7.646-39.791 5.352-43.617-2.294-3.826-8.41-6.122-17.585-10.714z"></path>
            </g>
            </svg></a>
          </div>
      
          <div class="call_btn">
            <a href="tel:971544888923"><svg style="pointer-events:none; display:block; height:40px; width:40px;" height="50px" width="50px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 511.995 511.995" xml:space="preserve">
             <circle style="fill:#00CC96;" cx="255.997" cy="255.997" r="255.997"></circle>
             <path style="fill:#07B587;" d="M510.112,287.098C495.896,404.491,402.27,497.34,284.432,510.445L139.495,365.508l29.543-10.106
               l11.772,4.443c42.537,44.203,85.408,89.961,119.615,76.633l11.328-26.655l-37.983-20.769L175.257,246.56l20.769-94.625
               l42.204,31.764c-0.222-4.776-1.666-10.551-9.219-14.438l21.657-107.176l70.414,35.762l189.14,189.14L510.112,287.098z"></path>
             <path style="fill:#F7F7F8;" d="M322.749,402.27c0.666,18.881-12.328,39.649-38.65,40.316
               c-32.874,0.666-74.079-52.199-102.955-75.634c-9.33-7.553-26.1-5.776-35.096,3.998l-6.553-5.442
               c14.216-15.326,32.985-17.215,49.646-4.998c25.656,20.88,70.192,73.523,95.625,72.857c19.991-0.777,28.766-19.214,27.211-31.541
               c3.665,0.444,7.219,0.555,10.884,0.555L322.749,402.27z"></path>
             <path style="fill:#FFD630;" d="M182.365,149.38c-21.324,14.327-34.873,35.207-33.763,60.196
               c3.332,78.299,100.512,192.25,175.479,195.581c18.214,0.777,36.317-8.997,50.09-22.879c17.437-8.219-50.645-78.077-63.528-72.19
               c-10.662,6.22-20.102,15.549-26.989,25.656c-17.881-12.55-64.306-69.415-72.857-88.74c11.217-4.887,22.213-12.328,30.321-21.546
               c8.219-11.439-47.313-91.627-58.752-76.189v0.111H182.365z"></path>
             <g>
               <path style="fill:#F7F7F8;" d="M305.312,199.58c0.777-2.332,1.555-4.665,2.11-6.997c4.998-20.102,1.777-40.649-9.108-57.53
                 c-10.885-16.881-28.21-28.544-48.424-32.43c-2.443-0.444-4.776-0.777-7.219-1l2.777-13.66c2.666,0.222,5.22,0.666,7.886,1.111
                 c23.99,4.554,44.425,18.214,57.197,38.205c12.772,19.991,16.659,44.314,10.773,67.971c-0.666,2.555-1.333,5.109-2.221,7.664
                 l-13.549-3.221L305.312,199.58z"></path>
               <path style="fill:#F7F7F8;" d="M282.433,194.137c0.555-1.555,1-3.11,1.444-4.776c3.554-14.327,1.222-28.988-6.442-40.982
                 c-7.664-12.105-20.102-20.324-34.54-23.101c-1.666-0.333-3.221-0.555-4.887-0.666l2.777-13.66c1.888,0.222,3.665,0.444,5.553,0.777
                 c18.103,3.443,33.541,13.771,43.204,28.877s12.55,33.43,8.108,51.422c-0.444,1.777-1,3.554-1.555,5.331l-13.549-3.221
                 L282.433,194.137L282.433,194.137z"></path>
               <path style="fill:#F7F7F8;" d="M260.331,188.918l0.222-0.666c2.332-9.219,0.777-18.658-4.221-26.433
                 c-4.998-7.775-12.883-13.105-22.213-14.882l-0.666-0.111l2.777-13.66l1.333,0.222c12.994,2.444,23.99,9.885,30.986,20.658
                 c6.886,10.773,8.997,23.99,5.776,36.762l-0.333,1.222l-13.549-3.221L260.331,188.918z"></path>
               <path style="fill:#F7F7F8;" d="M238.007,183.698c-0.111-2.999-1-5.887-2.666-8.441s-3.887-4.554-6.553-5.998l2.888-14.105
                 c6.553,2.221,12.105,6.442,15.771,12.328c3.665,5.776,5.22,12.661,4.554,19.436l-13.994-3.332V183.698z"></path>
               <path style="fill:#F7F7F8;" d="M330.857,205.577c0.777-2.221,1.333-4.554,1.888-6.775c6.775-27.211,2.332-55.087-12.328-77.966
                 c-14.66-22.99-38.206-38.65-65.638-43.87c-2.332-0.444-4.665-0.777-6.997-1.111l2.777-13.66c2.555,0.333,5.109,0.777,7.664,1.222
                 c31.208,5.998,57.753,23.768,74.412,49.757s21.657,57.641,13.994,88.406c-0.666,2.444-1.333,4.998-2.11,7.442l-13.549-3.221
                 L330.857,205.577z"></path>
             </g>
             </svg></a>
          </div>


        

        <!-- Optional JavaScript _____________________________  -->

        <!-- jQuery first, then Popper.js, then Bootstrap JS -->
        <!-- jQuery -->
        <script src="vendor/jquery.2.2.3.min.js"></script>
        <!-- Popper js -->
        <script src="vendor/popper.js/popper.min.js"></script>
        <!-- Bootstrap JS -->
        <script src="vendor/bootstrap/js/bootstrap.min.js"></script>
        <!-- Camera Slider -->
        <script src='vendor/Camera-master/scripts/jquery.mobile.customized.min.js'></script>
        <script src='vendor/Camera-master/scripts/jquery.easing.1.3.js'></script>
        <script src='vendor/Camera-master/scripts/camera.min.js'></script>
        <!-- menu  -->
        <script src="vendor/menu/src/js/jquery.slimmenu.js"></script>
        <!-- WOW js -->
        <script src="vendor/WOW-master/dist/wow.min.js"></script>
        <!-- owl.carousel -->
        <script src="vendor/owl-carousel/owl.carousel.min.js"></script>
        <!-- js count to -->
        <script src="vendor/jquery.appear.js"></script>
        <script src="vendor/jquery.countTo.js"></script>
        <!-- Fancybox -->
        <script src="vendor/fancybox/dist/jquery.fancybox.min.js"></script>

        <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery.isotope/3.0.6/isotope.pkgd.min.js"></script>
        <script>
            // external js: isotope.pkgd.js
            
            // init Isotope
            var $grid = $('.grid').isotope({
                itemSelector: '.color-shape',
                filter: '.health' 
            });
    
            // store filter for each group
            var filters = {};
            $('.filters').on('click', '.button', function (event) {
                var $button = $(event.currentTarget);
                // get group key
                var $buttonGroup = $button.parents('.button-group');
                var filterGroup = $buttonGroup.attr('data-filter-group');
                // set filter for group
                filters[filterGroup] = $button.attr('data-filter');
                // combine filters
                var filterValue = concatValues(filters);
                // set filter for Isotope
                $grid.isotope({ filter: filterValue });
            });
    
            // change is-checked class on buttons
            $('.button-group').each(function (i, buttonGroup) {
                var $buttonGroup = $(buttonGroup);
                $buttonGroup.on('click', 'button', function (event) {
                    $buttonGroup.find('.is-checked').removeClass('is-checked');
                    var $button = $(event.currentTarget);
                    $button.addClass('is-checked');
                });
            });
    
            // flatten object by concatting values
            function concatValues(obj) {
                var value = '';
                for (var prop in obj) {
                    value += obj[prop];
                }
                return value;
            }
        </script>

		<script>
			if ($(".clients-carousel").length) {
			$(".clients-carousel").owlCarousel({
				loop: true,
				margin: 10,
				nav: false,
				dots: false,
				smartSpeed: 100,
				autoplay: true,
				navText: ['<span class="fa fa-angle-left"></span>', '<span class="fa fa-angle-right"></span>'],
				responsive: {
					0: { items: 1 },
					480: { items: 2 },
					600: { items: 3 },
					768: { items: 4 },
					1023: { items: 4 }
				}
			});
		}

		if ($(".clients-carousel1").length) {
			$(".clients-carousel1").owlCarousel({
				loop: true,
				margin: 10,
				nav: false,
				dots: false,
				smartSpeed: 100,
				autoplay: true,
				autoplayTimeout: 2000,
				navText: ['<span class="fa fa-angle-left"></span>', '<span class="fa fa-angle-right"></span>'],
				responsive: {
					0: { items: 1 },
					480: { items: 2 },
					600: { items: 3 },
					768: { items: 5 },
					1023: { items: 5 }
				}
			});
		}

		if ($(".clients-carousel-two").length) {
			$(".clients-carousel-two").owlCarousel({
				loop: true,
				margin: 0,
				nav: false,
				smartSpeed: 100,
				autoplay: true,
				navText: ['<span class="fa fa-angle-left"></span>', '<span class="fa fa-angle-right"></span>'],
				responsive: {
					0: { items: 1 },
					480: { items: 2 },
					600: { items: 3 },
					991: { items: 4 },
					1200: { items: 5 },
					1400: { items: 6 }
				}
			});
		}

		if ($(".clients-carousel4").length) {
			$(".clients-carousel4").owlCarousel({
				loop: true,
				margin: 200,
				nav: false,
				smartSpeed: 100,
				autoplay: true,
				navText: ['<span class="fa fa-angle-left"></span>', '<span class="fa fa-angle-right"></span>'],
				responsive: {
					0: { items: 1 },
					480: { items: 2 },
					600: { items: 3 },
					768: { items: 3 },
					1023: { items: 3 }
				}
			});
		}

		</script>

        <!-- Theme js -->
        <script src="js/theme.js"></script>
    </div> <!-- /.main-page-wrapper -->
</body>

</html>